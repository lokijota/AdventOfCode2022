...
with open('Challenges\Exc19\input.txt') as f:
...
class GameState(object):
    def __init__(self, ores=0, clays=0, obsidians=0, geodes=0, orebots=1, claybots=0, obsidianbots=0, geodebots=0, constructingbot="", ticks=0, construction_history = []):
        ...
        self.ticks = ticks 
        self.construction_history = construction_history.copy()
...
for bpnumber in range(1, 31):
    ...
    max_scores_per_tick = np.zeros(30)

    # fastest_geobot = { }
    # fastest_geobot[0] = 25 # initialize -- no geobots built in 25 ticks / worse case
    # max_geobots = 0
    ...
    while len(game_queue) > 0:
        ...
        for construction in state.possible_construction(blueprints[bp_name]):
            ...
            if new_state.constructing == "GEB" and fastest_geobot > new_state.ticks:
                ...
                # if new_state.geodebots+1 > max_geobots:
                #     max_geobots = new_state.geodebots+1
                #     fastest_geobot[max_geobots] = new_state.ticks
                #     print("++ New record - At ticks", new_state.ticks, "built #geobots new max:", new_state.geodebots+1)
                # elif new_state.ticks < fastest_geobot[new_state.geodebots+1]:
                #     fastest_geobot[new_state.geodebots+1] = new_state.ticks
                #     print("+ Improved record - At ticks", new_state.ticks, "built #geobots:", new_state.geodebots+1)
            ...
            if new_state.ticks < 24: # no need to add to queue if we're already past 24 ticks
    ...
    print(">>>> Quality level with blueprint " + str(bpnumber) + " completed is " + str(quality_level) + ", checked # states: " + str(states_checked) + ", of which #bypassed=" + str(states_bypassed))
